...
export class ConfirmPage {
  ...
  ionViewWillEnter() {
    ...
    let B = this.bitcore;
    let networkName = (new B.Address(this.navParams.data.toAddress)).network.name;
    ...
    this.setWalletSelector(this.tx.network, this.tx.amount).then(() => {
  ...
  private setWalletSelector(network: string, minAmount: number): Promise<any> {
  ...
  private updateTx(tx: any, wallet: any, opts: any): Promise<any> {
    return new Promise((resolve, reject) => {
      ...
      this.feeProvider.getFeeRate(tx.network, this.usingMerchantFee ? maxAllowedMerchantFee : this.tx.feeLevel).then((feeRate: any) => {
  ...
  private showSendMaxWarning(wallet: any, sendMaxInfo: any): Promise<any> {
    return new Promise((resolve, reject) => {
      ...
      let msg = fee + " DGB will be deducted for digibyte networking fees."; // TODO: translate
  ...
  private verifyExcludedUtxos(wallet: any, sendMaxInfo: any): any {
    ...
    if (sendMaxInfo.utxosBelowFee > 0) {
      ...
      let message = "A total of " + amountBelowFeeStr + " DGB were excluded. These funds come from UTXOs smaller than the network fee provided."; // TODO: translate
      warningMsg.push(message);
    }

    if (sendMaxInfo.utxosAboveMaxSize > 0) {
      let amountAboveMaxSizeStr = (sendMaxInfo.amountAboveMaxSize / 1e8);
      let message = "A total of " + amountAboveMaxSizeStr + " DGB were excluded. The maximum size allowed for a transaction was exceeded."; // TODO: translate
      warningMsg.push(message);
    }
  ...
  public approve(tx: any, wallet: any): void {
    ...
    if (this.paymentExpired) {
      this.popupProvider.ionicAlert(null, this.translate.instant('This digibyte payment request has expired.'));
      return;
    }
