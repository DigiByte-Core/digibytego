...
export class AmountPage {
  ...
  private setAvailableUnits(): void {
    ...
    let hasWallets = this.profileProvider.getWallets().length;
  ...
  private fromFiat(val: any): number {
    return parseFloat((this.rateProvider.fromFiat(val, this.fiatCode) * this.satToUnit).toFixed(this.unitDecimals));
  }

  private toFiat(val: number): number {
    if (!this.rateProvider.getRate(this.fiatCode)) return;

    return parseFloat((this.rateProvider.toFiat(val * this.unitToSatoshi, this.fiatCode)).toFixed(2));
  }
