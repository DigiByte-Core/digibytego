...
export class ShapeshiftConfirmPage {
  ...
  private getSendMaxInfo(): Promise<any> {
    return new Promise((resolve, reject) => {
      ...
      this.feeProvider.getFeeRate(this.network, this.configWallet.settings.feeLevel || 'normal').then((feeRate: any) => {
  ...
  private satToFiat(coin: string, sat: number, isoCode: string): Promise<any> {
    return new Promise((resolve, reject) => {
      this.txFormatProvider.toFiat(sat, isoCode).then((value: any) => {
  ...
  private showSendMaxWarning(): Promise<any> {
    return new Promise((resolve, reject) => {
      ...
      let msg = fee + " " + this.fromWallet.coin.toUpperCase() + " will be deducted for digibyte networking fees."; // TODO: translate
        ...
        this.shapeshiftProvider.shift(data, (err: any, shapeData: any) => {
          ...
          this.createTx(this.fromWallet, toAddress).then((ctxp: any) => {
            ...
            this.shapeshiftProvider.getRate(this.getCoinPair(), (err: any, r: any) => {
              ...
              // Amount + Unit
              this.amountStr = this.txFormatProvider.formatAmountStr(ctxp.amount);
              this.withdrawalStr = this.txFormatProvider.formatAmountStr(withdrawalSat);
              this.feeStr = this.txFormatProvider.formatAmountStr(ctxp.fee);
              this.totalAmountStr = this.txFormatProvider.formatAmountStr(ctxp.amount + ctxp.fee);
