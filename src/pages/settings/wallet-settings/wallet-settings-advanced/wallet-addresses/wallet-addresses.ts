...
export class WalletAddressesPage {
  ...
  ionViewWillEnter() {
    ...
    this.walletProvider.getLowUtxos(this.wallet).then((resp) => {
      ...
      if (resp && resp.allUtxos && resp.allUtxos.length) {
        ...
        this.lowUtxosSum = this.txFormatProvider.formatAmountStr(_.sumBy(resp.lowUtxos || 0, 'satoshis'));
        this.allUtxosSum = this.txFormatProvider.formatAmountStr(allSum);
        this.minFee = this.txFormatProvider.formatAmountStr(resp.minFee || 0);
  ...
  public viewAllAddresses(): void {
    let modal = this.modalCtrl.create(AllAddressesPage, { noBalance: this.noBalance, withBalance: this.withBalance, walletName: this.wallet.name });
    modal.present();
  }
