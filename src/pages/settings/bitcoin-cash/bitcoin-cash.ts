...
export class BitcoinCashPage {
  ...
  ionViewWillEnter() {
    ...
    lodash.each(this.availableWallets, (wallet) => {
      this.walletProvider.getBalance(wallet, {}).then((balance) => {
        wallet.bchBalance = this.txFormatProvider.formatAmountStr(balance.availableAmount);
        wallet.error = null;
      }).catch((err) => {
  ...
  public duplicate(wallet: any) {
    ...
    this.walletProvider.getKeys(wallet).then((keys) => {
      ...
      importOrCreate().then((result: any) => {
        ...
        this.profileProvider.setWalletOrder(newWallet.credentials.walletId, null);
