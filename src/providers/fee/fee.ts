...
export class FeeProvider {
  ...
  public getFeeRate(network: string, feeLevel: string): Promise<any> {
    return new Promise((resolve, reject) => {
      if (feeLevel == 'custom') return resolve();
      network = network || 'livenet';
      this.getFeeLevels().then((response: any) => {
  ...
  public getCurrentFeeRate(network: string): Promise<any> {
    return new Promise((resolve, reject) => {
      this.getFeeRate(network, this.getCurrentFeeLevel()).then((data: number) => {
        return resolve(data)
      }).catch((err: any) => {
        return reject(err);
      });
  ...
  public getFeeLevels(): Promise<any> {
    return new Promise((resolve, reject) => {
      ...
      walletClient.getFeeLevels('livenet', (errLivenet, levelsLivenet) => {
        if (errLivenet) {
          return reject(this.translate.instant('Could not get dynamic fee'));
        }
        walletClient.getFeeLevels('testnet', (errTestnet, levelsTestnet) => {
          if (errTestnet) {
            return reject(this.translate.instant('Could not get dynamic fee'));
          }
