...
export class WalletProvider {
  ...
  public getStatus(wallet: any, opts: any): Promise<any> {
    return new Promise((resolve, reject) => {
      ...
      let cacheBalance = (wallet: any, balance: any): void => {
        ...
        // STR
        cache.totalBalanceStr = this.txFormatProvider.formatAmountStr(cache.totalBalanceSat);
        cache.lockedBalanceStr = this.txFormatProvider.formatAmountStr(cache.lockedBalanceSat);
        cache.availableBalanceStr = this.txFormatProvider.formatAmountStr(cache.availableBalanceSat);
        cache.spendableBalanceStr = this.txFormatProvider.formatAmountStr(cache.spendableAmount);
        cache.pendingBalanceStr = this.txFormatProvider.formatAmountStr(cache.pendingAmount);
        ...
        this.rateProvider.whenRatesAvailable().then(() => {
          ...
          let totalBalanceAlternative = this.rateProvider.toFiat(cache.totalBalanceSat, cache.alternativeIsoCode);
          let pendingBalanceAlternative = this.rateProvider.toFiat(cache.pendingAmount, cache.alternativeIsoCode);
          let lockedBalanceAlternative = this.rateProvider.toFiat(cache.lockedBalanceSat, cache.alternativeIsoCode);
          let spendableBalanceAlternative = this.rateProvider.toFiat(cache.spendableAmount, cache.alternativeIsoCode);
          let alternativeConversionRate = this.rateProvider.toFiat(100000000, cache.alternativeIsoCode);
      ...
      let fixTxsUnit = (txs: any): void => {
        ...
        if (cacheCoin == 'bits') {
          ...
          lodash.each(txs, (tx: any) => {
            tx.amountStr = this.txFormatProvider.formatAmountStr(tx.amount);
            tx.feeStr = this.txFormatProvider.formatAmountStr(tx.fees);
          });
        };
      };
  ...
  public getMinFee(wallet: any, nbOutputs?: number): Promise<any> {
    return new Promise((resolve, reject) => {
      this.feeProvider.getFeeLevels().then((data: any) => {
  ...
  public copyCopayers(wallet: any, newWallet: any): Promise<any> {
    return new Promise((resolve, reject) => {
      ...
      lodash.each(wallet.credentials.publicKeyRing, (item) => {
        ...
        newWallet._doJoinWallet(newWallet.credentials.walletId, walletPrivKey, item.xPubKey, item.requestPubKey, name, {}, (err: any) => {
